...
/// Makes the restored macOS titlebar area behave like a native drag region.
/// The live mouse-down must start the drag; waiting for egui's threshold loses
/// the event before AppKit receives it. Other platforms use their native
/// titlebar and need no overlay.
pub fn titlebar_drag(ui: &mut egui::Ui, rect: egui::Rect) {
    ...
    if cfg!(windows) && response.double_clicked() {
        ...
        let drag_started = (cfg!(windows) && response.drag_started())
            || (cfg!(target_os = "macos")
...
/// Draws Windows' caption controls over the outermost top-right header.
...
mod window_chrome_tests {
    ...
    fn resize_hit_test_covers_edges_and_corners() {
        use egui::ResizeDirection as Direction;

        let window = Rect::from_min_max(egui::pos2(20.0, 30.0), egui::pos2(120.0, 110.0));
        for (position, expected) in [
            (egui::pos2(21.0, 31.0), Some(Direction::NorthWest)),
            (egui::pos2(70.0, 31.0), Some(Direction::North)),
            (egui::pos2(119.0, 31.0), Some(Direction::NorthEast)),
            (egui::pos2(21.0, 70.0), Some(Direction::West)),
            (egui::pos2(70.0, 70.0), None),
            (egui::pos2(119.0, 70.0), Some(Direction::East)),
            (egui::pos2(21.0, 109.0), Some(Direction::SouthWest)),
            (egui::pos2(70.0, 109.0), Some(Direction::South)),
            (egui::pos2(119.0, 109.0), Some(Direction::SouthEast)),
        ] {
            assert_eq!(
